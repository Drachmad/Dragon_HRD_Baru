<?php
foreach ($transaksi_tunjangan_jasa as $rowh) {
};
?>
...
<div class="container-fluid">
	...
	<form id="transaksitunjanganjasa" name="transaksitunjanganjasa" action="<?php echo base_url('admin/Transaksi_Tunjangan_Jasa/update_aksi'); ?>" class="form-horizontal needs-validation" method="post" novalidate>
		<div class="form-body">
			<div class="row">
				<div class="col-md-12">
				...
				<div class="table-responsive scrollable">
					<table id="datatable" class="table table-hoverx table-stripedx table-borderedx table-condensed table-scrollable">
						...
						<tbody>
							<?php
							$no = 0;
							foreach ($transaksi_tunjangan_jasa as $row) :
							?>
								<tr>
									...
									<td><input name="JASA[]" onkeyup="hitung()" id="JASA<?php echo $no; ?>" value="<?php echo number_format($row->JASA, 2, '.', ','); ?>" type="text" class="form-control JASA rightJustified text-primary"></td>
									<td>
						...
						<tfoot>
							...
							<td></td>
							<td></td>
							<td><input class="form-control TJASA rightJustified text-primary font-weight-bold" id="TJASA" name="TJASA" value="<?php echo number_format($rowh->TJASA, 2, '.', ','); ?>" readonly></td>
							<td></td>
						</tfoot>
					</table>
				</div>
...
<script>
	...
	$(document).ready(function() {
		$("#TJASA").autoNumeric('init', {
			aSign: '<?php echo ''; ?>',
			vMin: '-999999999.99'
		});
		jumlahdata = 100;
		for (i = 0; i <= jumlahdata; i++) {
			$("#JASA" + i.toString()).autoNumeric('init', {
				aSign: '<?php echo ''; ?>',
				vMin: '-999999999.99'
			});
		}
	...
	function hitung() {
		...
		if (isNaN(TJASA)) TJASA = 0;

		$('#TJASA').val(numberWithCommas(TJASA));

		$('#TJASA').autoNumeric('update');
	}
	...
	function tambah() {
		...
		td5.innerHTML = "<input name='JASA[]' onclick='select()' onkeyup='hitung()' value='0' id=JASA" + idrow + " type='text' class='form-control JASA rightJustified text-primary'>";
		td6.innerHTML = "<input type='hidden' name='NO_ID[]' value='0' id=NO_ID" + idrow + "  class='form-control'>" +
...
<script>
	...
	function select_nm_peg() {
		$('.js-example-responsive-nm_peg').select2({
			ajax: {
				url: "<?= base_url('admin/Transaksi_Tunjangan_Jasa/getDataAjax_Pegawai') ?>",
				dataType: "json",
				type: "post",
				delay: 10,
