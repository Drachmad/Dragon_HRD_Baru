<?php
foreach ($transaksi_tunjangan_nikah_mati as $rowh) {
};
?>
...
<div class="container-fluid">
	...
	<form id="transaksitunjanganobat" name="transaksitunjanganobat" action="<?php echo base_url('admin/Transaksi_Tunjangan_Obat/update_aksi'); ?>" class="form-horizontal needs-validation" method="post" novalidate>
		<div class="form-body">
			<div class="row">
				<div class="col-md-12">
				...
				<div class="table-responsive scrollable">
					<table id="datatable" class="table table-hoverx table-stripedx table-borderedx table-condensed table-scrollable">
						...
						<tbody>
							<?php
							$no = 0;
							foreach ($transaksi_tunjangan_obat as $row) :
							?>
								<tr>
									...
									<td><input name="OBAT[]" onkeyup="hitung()" id="OBAT<?php echo $no; ?>" value="<?php echo number_format($row->OBAT, 2, '.', ','); ?>" type="text" class="form-control OBAT rightJustified text-primary"></td>
									<td>
						...
						<tfoot>
							...
							<td></td>
							<td></td>
							<td><input class="form-control TOBAT rightJustified text-primary font-weight-bold" id="TOBAT" name="TOBAT" value="<?php echo number_format($rowh->T_NIKAH_MATI, 2, '.', ','); ?>" readonly></td>
							<td></td>
						</tfoot>
					</table>
				</div>
...
<script>
	...
	$(document).ready(function() {
		$("#TOBAT").autoNumeric('init', {
			aSign: '<?php echo ''; ?>',
			vMin: '-999999999.99'
		});
		jumlahdata = 100;
		for (i = 0; i <= jumlahdata; i++) {
			$("#OBAT" + i.toString()).autoNumeric('init', {
				aSign: '<?php echo ''; ?>',
				vMin: '-999999999.99'
			});
		}
	...
	function hitung() {
		...
		if (isNaN(TOBAT)) TOBAT = 0;

		$('#TOBAT').val(numberWithCommas(TOBAT));

		$('#TOBAT').autoNumeric('update');
	}
	...
	function tambah() {
		...
		td5.innerHTML = "<input name='OBAT[]' onclick='select()' onkeyup='hitung()' value='0' id=OBAT" + idrow + " type='text' class='form-control OBAT rightJustified text-primary'>";
		td6.innerHTML = "<input type='hidden' name='NO_ID[]' id=NO_ID" + idrow + "  class='form-control' value='0'>" +
...
<script>
	...
	function select_nm_peg() {
		$('.js-example-responsive-nm_peg').select2({
			ajax: {
				url: "<?= base_url('admin/Transaksi_Tunjangan_Obat/getDataAjax_Pegawai') ?>",
				dataType: "json",
				type: "post",
				delay: 10,
