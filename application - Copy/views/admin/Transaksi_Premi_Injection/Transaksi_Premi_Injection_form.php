...
<div class="container-fluid">
	...
	<form id="premiinjection" name="premiinjection" action="<?php echo base_url('admin/Transaksi_Premi_Injection/input_aksi'); ?>" class="form-horizontal needs-validation" method="post" novalidate>
		<div class="form-body">
			<div class="row">
				<div class="col-md-12">
...
<script>
	...
	$(document).ready(function() {
		...
		$('body').on('click', '.select_kik_grup', function() {
			...
			$.ajax({
				type: 'get',
				url: '<?php echo base_url('index.php/admin/Transaksi_Premi_Injection/filter_kik_grup'); ?>',
				data: {
					kik_grup: kik_grup
				},
				dataType: 'json',
				success: function(response) {
					...
					for (i = 0; i < response.length; i++) {
						html += '<tr>' +
							...
							'<td><input name="TMS[]" onclick="select()" onkeyup="hitung()" value="27" id=TMS' + i + ' type="text" class="form-control TMS rightJustified text-primary"></td>' +
							'<td><input name="MSMN[]" onclick="select()" onkeyup="hitung()" value="27" id=MSMN' + i + ' type="text" class="form-control MSMN rightJustified text-primary"></td>' +
	...
	function hitung() {
		...
		for (i = 0; i < total_row; i++) {
			...
			var nbmaint1 = Math.round((0.09 * msmn), 2);
			var nbmaint2 = 0;
			var wkmag = (0.020 * tms).toFixed(2);
			//			var kaqc = (0.015*tms).toFixed(2);
			var kaqc = (Math.round(0.015 * tms * 100) / 100).toFixed(2);
