<?php
foreach ($transaksi_uml_borongan as $rowh) {
};
?>
...
<div class="container-fluid">
	...
	<form id="transaksiumlborongan" name="transaksiumlborongan" action="<?php echo base_url('admin/Transaksi_UML_Borongan/update_aksi'); ?>" class="form-horizontal needs-validation" method="post" novalidate>
		<div class="form-body">
			<div class="row">
				<div class="col-md-12">
				...
				<div class="table-responsive scrollable">
					<table id="datatable" class="table table-hoverx table-stripedx table-borderedx table-condensed table-scrollable">
						...
						<tbody>
							<?php
							$no = 0;
							foreach ($transaksi_uml_borongan as $row) :
							?>
								<tr>
									...
									<td><input name="ULEMBUR[]" onkeyup="hitung()" id="ULEMBUR<?php echo $no; ?>" value="<?php echo number_format($row->ULEMBUR, 2, '.', ','); ?>" type="text" class="form-control ULEMBUR rightJustified text-primary" readonly></td>
									<td>
						...
						<tfoot>
							...
							<td></td>
							<td></td>
							<td><input class="form-control T_ULEMBUR rightJustified text-primary font-weight-bold" id="T_ULEMBUR" name="T_ULEMBUR" value="<?php echo number_format($rowh->T_ULEMBUR, 2, '.', ','); ?>" readonly></td>
							<td></td>
						</tfoot>
					</table>
				</div>
...
<script>
	...
	$(document).ready(function() {
		$("#T_ULEMBUR").autoNumeric('init', {
			aSign: '<?php echo ''; ?>',
			vMin: '-999999999.99'
		});
		jumlahdata = 100;
		for (i = 0; i <= jumlahdata; i++) {
			$("#ULEMBUR" + i.toString()).autoNumeric('init', {
				aSign: '<?php echo ''; ?>',
				vMin: '-999999999.99'
			});
		}
	...
	function hitung() {
		...
		$(".ULEMBUR").each(function() {
			var val = parseFloat($(this).val().replace(/,/g, ''));
			if (isNaN(val)) val = 0;
			T_ULEMBUR += val;
		});

		if (isNaN(T_ULEMBUR)) T_ULEMBUR = 0;

		$('#T_ULEMBUR').val(numberWithCommas(T_ULEMBUR));

		$('#T_ULEMBUR').autoNumeric('update');
	}
	...
	function tambah() {
		...
		td3.innerHTML = "<input name='PT[]' id=PT0" + idrow + " type='text' class='form-control PT text_input' readonly>";
		td4.innerHTML = "<input name='ULEMBUR[]' onclick='select()' onkeyup='hitung()' value='0' id=ULEMBUR" + idrow + " type='text' class='form-control ULEMBUR rightJustified text-primary'>";
		td5.innerHTML = "<input type='hidden' name='NO_ID[]' id=NO_ID" + idrow + "  class='form-control' value='0'>" +
...
<script>
	...
	function select_nm_peg() {
		$('.js-example-responsive-nm_peg').select2({
			ajax: {
				url: "<?= base_url('admin/Transaksi_UML_Borongan/getDataAjax_Pegawai') ?>",
				dataType: "json",
				type: "post",
				delay: 10,
