 	  ...
 	  columnDefs: [{
		...
		rowGroup: {
            ...
            endRender: function ( rows, group ) {
                ...
                sum = $.fn.dataTable.render.number(',', '.', 2).display( sum );
 
                var total = rows
                    .data()
                    .pluck(7)
                    .reduce( function (a, b) {
											return parseFloat(a) + parseFloat(b.replace(/[^\d]/g, ""), '')*1;
                    }, 0);
                total = $.fn.dataTable.render.number(',', '.', 0).display( total );
 
                return $('<tr/>')
                    .append( '<td colspan="3">Total untuk '+group+'</td>' )
                    .append( '<td/>' )
										.append( '<td/>' )
                    .append( '<td/>' )
                    .append( '<td style="text-align:right;" >'+sum+'</td>' )
