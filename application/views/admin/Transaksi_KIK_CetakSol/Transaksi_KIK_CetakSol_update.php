...
<div class="container-fluid">
	...
	<form id="kikcetaksol" name="kikcetaksol" action="<?php echo base_url('admin/Transaksi_KIK_CetakSol/update_aksi'); ?>" class="form-horizontal needs-validation" method="post" novalidate>
		<div class="form-body">
			<div class="row">
				<div class="col-md-12">
					...
					<table id="datatable" class="table table-hoverx table-stripedx table-borderedx table-condensed table-scrollable">
						<thead>
							<tr>
								...
								<th width="75px">KR IN</th>
								<th width="75px">KR PSP</th>
								<th width="75px">WK IN</th>
								<th width="75px">ORG</th>
								<th width="75px">PSP</th>
								<th width="75px">HR</th>
								<th width="50px"></th>
							</tr>
						</thead>
						...
						?>
							...
							<td><input name="REC[]" id="REC<?php echo $no; ?>" value="<?= $row->REC ?>" type="text" class="form-control REC text_input" onkeypress="return tabE(this,event)" readonly></td>
								...
								<td><input name="ITEM[]" id="ITEM<?php echo $no; ?>" value="<?= $row->ITEM ?>" type="text" class="form-control ITEM text_input" readonly></td>
								<td><input name="DES1[]" id="DES1<?php echo $no; ?>" value="<?= $row->DES1 ?>" type="text" class="form-control DES1 text_input" readonly></td>
								...
								<td><input name="KA[]" onclick="select()" onkeyup="hitung()" id="KA<?php echo $no; ?>" value="<?php echo number_format($row->KA, 2, '.', ','); ?>" type="text" class="form-control KA rightJustified text-primary"></td>
								<td><input name="KA_PRS[]" onclick="select()" onkeyup="hitung()" id="KA_PRS<?php echo $no; ?>" value="<?php echo number_format($row->KA_PRS, 2, '.', ','); ?>" type="text" class="form-control KA_PRS rightJustified text-primary"></td>
								<td><input name="WA[]" onclick="select()" onkeyup="hitung()" id="WA<?php echo $no; ?>" value="<?php echo number_format($row->WA, 2, '.', ','); ?>" type="text" class="form-control WA rightJustified text-primary"></td>
								...
								<td><input name="PRS[]" onclick="select()" onkeyup="hitung()" id="PRS<?php echo $no; ?>" value="<?php echo number_format($row->PRS, 2, '.', ','); ?>" type="text" class="form-control PRS rightJustified text-primary"></td>
						...
						<tfoot>
							...
							<td></td>
							<td></td>
							<td><input class="form-control TQTY rightJustified text-primary font-weight-bold" id="TQTY" name="TQTY" value="<?php echo number_format($rowh->TQTY, 2, '.', ','); ?>" readonly></td>
							<td></td>
		...
		<div class="col-md-12">
			<div class="form-group row">
				<div class="col-md-1 ">						
					<input class="form-control rightJustified text-danger font-weight-bold" id="" name="" readonly>
				</div>
				<div class="col-md-1 ">
					<label class="label">Subsidi </label>
				</div>
				<div class="col-md-1">						
					<input class="form-control rightJustified text-danger font-weight-bold" id="" name="" readonly>
				</div>
		...
		<div class="col-md-12">
			<div class="form-group row">
				...
				<div class="col-md-2 ">
					<input class="form-control LUNAS_BS rightJustified text-danger font-weight-bold" onclick="select()" onkeyup="hitung()" id="LUNAS_BS" name="LUNAS_BS" value="<?php echo number_format($rowh->LUNAS_BS, 2, '.', ','); ?>">
				</div>
			</div>
		</div>
		<div class="col-md-12">
		...
		<div class="col-md-12">
			<div class="form-group row">
				...
				<div class="col-md-1 ">
					<label class="label">Lembur Jam</label>
				</div>
				<div class="col-md-2 ">						
					<input class="form-control LEM_JAM rightJustified text-primary font-weight-bold" onclick="select()" onkeyup="hitung()" id="LEM_JAM" name="LEM_JAM" value="<?php echo number_format($rowh->LEM_JAM, 2, '.', ','); ?>">
				</div>
			</div>
		</div>
		<div class="col-md-12">
...
<script>
	...
	$(document).ready(function() {
		...
		$("#LEM_JAM").autoNumeric('init', {aSign: '<?php echo ''; ?>',vMin: '-999999999.99'});
		...
		for (i = 0; i <= jumlahdata; i++) {
			...
			$("#KA" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
			$("#KA_PRS" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
			$("#WA" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
			$("#ORG" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
			$("#PRS" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
	...
	function hitung() {
		...
		var LEM_JAM = parseFloat($('#LEM_JAM').val().replace(/,/g, ''));
		var PPN = 0;
		var MINUSS = 0;
		var POT_BON = 0;
		...
		for (i=0;i<total_row;i++) {
			...
			var ka = parseFloat($('#KA'+i).val().replace(/,/g, ''));
			var ka_prs = parseFloat($('#KA_PRS'+i).val().replace(/,/g, ''));
			var wa = parseFloat($('#WA'+i).val().replace(/,/g, ''));
			var org = parseFloat($('#ORG'+i).val().replace(/,/g, ''));
			var prs = parseFloat($('#PRS'+i).val().replace(/,/g, ''));
		...
		for (i=0;i<total_row;i++) {
			...
			if (tanggal <= '20191231' && 
					...
					'721.03.D')) 
			{
				hr = ((org*95998)+(wa*97000)+(ka*25000)+(ka_prs*4762)+(prs*13714))-jumlah;
				hr = hr.toFixed(2);
				if(isNaN(hr)) hr = 0;
			...
			if (tanggal <= '20191231' && 
					...
					'721.07.AB')) 
			{
				hr = ((org*95998)+(wa*97000)+(ka*25000)+(ka_prs*625)+(prs*47999))-jumlah;
				hr = hr.toFixed(2);
				if(isNaN(hr)) hr = 0;
				$('#HR'+i).val(numberWithCommas(hr));
			}
			...
			if (tanggal > '20191231' && 
					...
					'721.03.D')) 
			{
				hr = ((org*104167)+(wa*109400)+(ka*27604)+(ka_prs*5258)+(prs*14881))-jumlah;
				hr = hr.toFixed(2);
				if(isNaN(hr)) hr = 0;
				$('#HR'+i).val(numberWithCommas(hr));
			}
			if (tanggal > '20191231' && 
					...
					'721.10.AB')) 
			{
				hr = ((org*104167)+(wa*109400)+(ka*27604)+(ka_prs*6901)+(prs*52084))-jumlah;
				hr = hr.toFixed(2);
				if(isNaN(hr)) hr = 0;
				$('#HR'+i).val(numberWithCommas(hr));
			}
		...
		POT_BON = MINUSS + UPAH_TAMBAH - LUNAS_BS - LEM_JAM;
		...
		if(isNaN(LEM_JAM)) LEM_JAM = 0;

		$('#UMR').val(numberWithCommas(UMR));
		...
		$('#LEM_JAM').val(numberWithCommas(LEM_JAM));

		$("#UMR").autoNumeric('update');
	...
	function tambah() {
		...
		var no_kik0 = "<div class='input-group'><select class='js-example-responsive-no_kik form-control NO_KIK text_input' name='NO_KIK[]' value='' id=NO_KIK" + idrow + " onchange='no_kik(this.id)' onfocusout='hitung()' required></select></div>";
		var no_kik = no_kik0;
		...
		td3.innerHTML = "<input name='TGL_KIK[]' id=TGL_KIK" + idrow + " type='text' class='date form-control TGL_KIK text_input' data-date-format='dd-mm-yyyy' value='<?php if(isset($_POST["tampilkan"])) { echo $_POST["TGL_KIK"]; } else echo date('d-m-Y'); ?>'>";
		// td4.innerHTML = model;
		...
		td10.innerHTML = "<input name='KA[]' onkeyup='hitung()' onclick='select()' value='0' id=KA" + idrow + " type='text' class='form-control KA rightJustified text-primary'>";
		td11.innerHTML = "<input name='KA_PRS[]' onkeyup='hitung()' onclick='select()' value='0' id=KA_PRS" + idrow + " type='text' class='form-control KA_PRS rightJustified text-primary'>";
		td12.innerHTML = "<input name='WA[]' onkeyup='hitung()' onclick='select()' value='0' id=WA" + idrow + " type='text' class='form-control WA rightJustified text-primary'>";
		td13.innerHTML = "<input name='ORG[]' onkeyup='hitung()' onclick='select()' value='0' id=ORG" + idrow + " type='text' class='form-control ORG rightJustified text-primary'>";
		td14.innerHTML = "<input name='PRS[]' onkeyup='hitung()' onclick='select()' value='0' id=PRS" + idrow + " type='text' class='form-control PRS rightJustified text-primary'>";
		td15.innerHTML = "<input name='HR[]' onkeyup='hitung()' value='0' id=HR" + idrow + " type='text' class='form-control HR rightJustified text-primary' readonly>";
...
// NO_KIK
	function select_no_kik() {
		...
		$('.js-example-responsive-no_kik').select2({
			ajax: {
				url: "<?= base_url('admin/Transaksi_KIK_CetakSol/getDataAjax_KIK2') ?>",
				dataType: "json",
				type: "post",
				delay: 10,
