				...
				<div class="table-responsive scrollable">
					<table id="datatable" class="table table-hoverx table-stripedx table-borderedx table-condensed table-scrollable">
						<thead>
							<tr>
								<th width="50px">No</th>
								<th width="400px">NIP</th>
								<th width="370px">Nama</th>
								<th width="440px">Jumlah</th>
								<th width="50px"></th>
							</tr>
						</thead>
						<tbody>
						...
						?>
							<tr>
								...
								<td><input name="ULEMBUR[]" onkeyup="hitung()" id="ULEMBUR<?php echo $no; ?>" value="<?php echo number_format($row->ULEMBUR, 2, '.', ','); ?>" type="text" class="form-control ULEMBUR rightJustified text-primary"></td>
								<td>
						...
						<tfoot>
							...
							<td></td>
							<td></td>
							<td><input class="form-control T_ULEMBUR rightJustified text-primary font-weight-bold" id="T_ULEMBUR" name="T_ULEMBUR" value="<?php echo number_format($row->T_ULEMBUR, 2, '.', ','); ?>" readonly></td>
							<td></td>
						</tfoot>
					</table>
				</div>
...
<script>
	...
	$(document).ready(function() {
		$("#T_ULEMBUR").autoNumeric('init', {aSign: '<?php echo ''; ?>',vMin: '-999999999.99'});
		jumlahdata = 100;
		for (i = 0; i <= jumlahdata; i++) {
			$("#ULEMBUR" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
		}
		//MyModal Bagian
		...
		$('body').on('click', '.select_kd_bag', function() {
			...
			$.ajax({
				type:'get',
				url : '<?php echo base_url('index.php/admin/Transaksi_UML_Per_Jam/filter_kd_bag'); ?>',
				data:{ kd_bag : kd_bag},
				dataType: 'json',
				...
				// alert(response);
                    ...
                    for(i=0; i<response.length; i++){
                        html += '<tr>'+
									...
									'</td>'+
									'<td><input name="ULEMBUR[]" onclick="select()" onkeyup="hitung()" value="0" id=ULEMBUR'+i+' type="text" class="form-control ULEMBUR rightJustified text-primary"></td>'+
	...
	function tambah() {
		...
		td5.innerHTML = "<input type='hidden' name='NO_ID[]' value='0' id=NO_ID" + idrow + "  class='form-control'>" +
