...
<script>
	...
	$(document).ready(function() {
		...
		$("#PREMI").autoNumeric('init', {aSign: '<?php echo ''; ?>',vMin: '-999999999.99'});
		...
		$("#PK").autoNumeric('init', {aSign: '<?php echo ''; ?>',vMin: '-999999999.99'});
		$("#PKPH").autoNumeric('init', {aSign: '<?php echo ''; ?>',vMin: '-999999999.99'});
		...
		for (i = 0; i <= jumlahdata; i++) {
			...
			$("#BOR1" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
		}
		//MyModal Bagian
				...
				// alert(response);
                    ...
                    for(i=0; i<response.length; i++){
                        html += '<tr>'+
									...
									'<td><input name="TOTAL[]" onchange="hitung()" id=TOTAL'+i+' value="'+numberWithCommas(response[i].TOTAL)+'" type="text" class="form-control TOTAL rightJustified text-primary" readonly></td>'+
					...
					for(i=0; i<=jumlahdata; i++){
						...
						$("#BOR1" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
						$("#BOR2" + i.toString()).autoNumeric('init', {aSign: '<?php echo ''; ?>', vMin: '-999999999.99'});
					}
				}
			});
		});
