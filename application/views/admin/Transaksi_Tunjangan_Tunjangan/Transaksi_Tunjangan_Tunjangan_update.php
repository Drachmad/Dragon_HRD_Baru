<?php
foreach ($tunjangan_tunjangan as $rowh) {
};
?>
...
<div class="container-fluid">
	...
	<form id="transaksitunjangantunjangan" name="transaksitunjangantunjangan" action="<?php echo base_url('admin/Transaksi_Tunjangan_Tunjangan/update_aksi'); ?>" class="form-horizontal needs-validation" method="post" novalidate>
		<div class="form-body">
			<div class="row">
				<div class="col-md-12">
				...
				<div class="table-responsive scrollable">
					<table id="datatable" class="table table-hoverx table-stripedx table-borderedx table-condensed table-scrollable">
						...
						<tbody>
							<?php
							$no = 0;
							foreach ($tunjangan_tunjangan as $row) :
							?>
								<tr>
									...
									<td><input name="TJABATAN[]" onkeyup="hitung()" id="TJABATAN<?php echo $no; ?>" value="<?php echo number_format($row->TJABATAN, 2, '.', ','); ?>" type="text" class="form-control TJABATAN rightJustified text-primary"></td>
									<td>
						...
						<tfoot>
							...
							<td></td>
							<td></td>
							<td><input class="form-control T_TJABATAN rightJustified text-primary font-weight-bold" id="T_TJABATAN" name="T_TJABATAN" value="<?php echo number_format($row->T_TJABATAN, 2, '.', ','); ?>" readonly></td>
							<td></td>
						</tfoot>
					</table>
				</div>
...
<script>
	...
	$(document).ready(function() {
		...
		$('body').on('click', '.select_kd_bag', function() {
			...
			$.ajax({
				type: 'get',
				url: '<?php echo base_url('index.php/admin/Transaksi_Tunjangan_Tunjangan/filter_kd_bag'); ?>',
				data: {
					kd_bag: kd_bag
				},
				dataType: 'json',
				success: function(response) {
					...
					for (i = 0; i < response.length; i++) {
						html += '<tr>' +
							...
							'</td>' +
							'<td><input name="TJABATAN[]" onkeyup="hitung()" value="0" id=TJABATAN' + i + ' type="text" class="form-control TJABATAN rightJustified text-primary"></td>' +
	...
	function hitung() {
		...
		$(".TJABATAN").each(function() {
			var val = parseFloat($(this).val().replace(/,/g, ''));
			if (isNaN(val)) val = 0;
			T_TJABATAN += val;
		});

		if (isNaN(T_TJABATAN)) T_TJABATAN = 0;

		$('#T_TJABATAN').val(numberWithCommas(T_TJABATAN));

		$('#T_TJABATAN').autoNumeric('update');
	}
	...
	function tambah() {
		...
		var nm_peg0 = "<div class='input-group'><select class='js-example-responsive-nm_peg form-control NM_PEG0' name='NM_PEG[]' id=NM_PEG0 text_input" + idrow + " onchange='nm_peg(this.id)' onfocusout='hitung()' required></select></div>";
...
<script>
	...
	function select_nm_peg() {
		$('.js-example-responsive-nm_peg').select2({
			ajax: {
				url: "<?= base_url('admin/Transaksi_Tunjangan_Tunjangan/getDataAjax_Pegawai') ?>",
				dataType: "json",
				type: "post",
				delay: 10,
